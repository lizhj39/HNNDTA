...
def smiles2mpnnfeature(smiles):
	...
	'''
		data_output-flow:
			...
			DBTA: train(): data_output.DataLoader(data_process_loader())
			mpnn_collate_func()
...
def data_process(X_drug = None, X_target = None, y=None, drug_encoding=None, target_encoding=None, 
	...
	if sample_frac != 1:
		...
		print('after subsample: ' + str(len(df_data)) + ' data_output points...')
	...
	if DTI_flag:
		...
		if split_method == 'random': 
			...
			print('do not do train/test split on the data_output for already splitted data_output')
...
class data_process_loader(data.Dataset):
	...
	def __getitem__(self, index):
		'Generates one sample of data_output'
