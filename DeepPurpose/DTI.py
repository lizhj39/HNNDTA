...
class DBTA:
	...
	def test_(self, data_generator, model, repurposing_mode = False, test = False):
		...
		if self.binary:
			...
			if repurposing_mode:
				return y_pred
			return mean_squared_error(y_label, y_pred), pearsonr(y_label, y_pred)[0], pearsonr(y_label, y_pred)[1], concordance_index(y_label, y_pred), y_pred, y_label, loss
	...
	def train(self, train, val = None, test = None, verbose = True):
			...
			if val is not None:
				...
				with torch.set_grad_enabled(False):
					if self.binary:  
						...
						mse, r2, p_val, CI, logits, y_label, loss_val = self.test_(validation_generator, self.model)
		...
		if test is not None:
			...
			if self.binary:
				...
				mse, r2, p_val, CI, logits, y_label, loss_test = self.test_(testing_generator, model_max)
				np.save(r"tmp_files/tmp_evaluations.npy", np.array([mse, CI]))
				# np.savetxt("tmp_pred_label.csv", np.array([logits, y_label]), delimiter=",")
	...
	def load_pretrained(self, path):
		...
		# to support training from multi-gpus data_output-parallel:
        
		if next(iter(state_dict))[:7] == 'module.':
			# the pretrained model is from data_output-parallel module
